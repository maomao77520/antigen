            ...
            <view class="flex-col section_1">
                <view class="center-group justify-between" v-if="stype != 'person'">
                    ...
                    <view class="flex-row group_18">
                        <input type="text" v-model="qrcode" class="qrcodeinput"
                               placeholder-class="placeholder_qrcode" placeholder="扫描单位检测二维码" :disabled="true"/>
                ...
                <view class="center-group justify-between">
                    <view class="left-group">
                        <text class="text_6">*</text>
                        <text class="text_8">单位名称</text>
                    </view>
                    <input class="text_input" type="text" v-model="obj.unitName" disabled
                           placeholder-class="placeholder_qrcode" placeholder="扫码后读取单位名称"/>
                </view>
                ...
                <view class="flex-col group_9">
                    ...
                    <view class="flex-col">
                        <view class="center-group justify-between">
                            ...
                            <text class="text_10">{{ isMeObj.idType }}</text>
                        </view>
                        <view class="center-group justify-between">
                            <view class="left-group">
                                <text class="text_6">*</text>
						...
						if (r) {
							ServiceApi.userInfo().then(res => {
							    if (res.data.status == 200) {
									// #ifdef MP-ALIPAY
									monitor.api({api:"userInfo",success:true,c1:"taSR",time:0})
									// #endif
									...
									// #ifdef MP-ALIPAY
									monitor.api({api:"userInfo",success:false,c1:"taSR",time:0})
									// #endif
			...
			getPlaceNameByBatchNo(batchNo){
				...
				ServiceApi.getPlaceNameByBatchNo(batchNo).then(resp=>{
					if(resp.data.status==200){
						...
						// #endif
						that.obj.unitName = resp.data.result.placeName
						that.isMeObj.unitName = resp.data.result.placeName
						that.notMeObj.unitName = resp.data.result.placeName
					}else{
						// #ifdef MP-ALIPAY
